...
interface MacbookModelProps {
  /** Already-loaded THREE.Texture — never changes the hook call order */
  texture: THREE.Texture;
  [key: string]: any;
}

export default function MacbookModel({ texture, ...props }: MacbookModelProps) {
  const { nodes, materials } = useGLTF('/models/macbook-transformed.glb') as any;
  ...
  return (
    <group {...props} dispose={null}>
      ...
      <mesh geometry={nodes.Object_123.geometry} rotation={[Math.PI / 2, 0, 0]}>
        {/* texture is already loaded — switching it never suspends */}
